...
class Spotify_thingy:
    def __init__(self, methods, artists_names, song_quantity, client_id, client_secret):
    ...
    def search_and_add(self):
        ...
        if self.methods == 'multiple pages':
            for artist_list in self.artists_names:
            ...
            msg = CTkMessagebox(title="Hello!", message=f"Those are a lot of songs are you sure you wanna add all of them to a playlist?",
                        icon="question", option_1="Yes", option_2="No")
            response = msg.get()

            if response=="Yes":
                for each_item in self.song_list:
                    print(each_item)
